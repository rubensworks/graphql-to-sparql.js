...
import type { DirectiveNode, NameNode } from 'graphql';
import { JsonLdContextNormalized } from 'jsonld-context-parser';
import { DataFactory } from 'rdf-data-factory';
import { Converter } from '../lib/Converter';
import type { IDirectiveNodeHandlerOutput } from '../lib/handler/directivenode';
...
describe('Util', () => {
  ...
  describe('#joinOperations', () => {
    it('should (not) error on no patterns', async() => {
      expect(util.joinOperations([])).toEqual(util.operationFactory.createBgp([]));
    });

    it('should return the single passed operation', async() => {
      expect(util.joinOperations([
    ...
    it('should return a left join from multiple BGPs and one left join', async() => {
      expect(util.joinOperations([
        OperationFactory.createLeftJoin(
          OperationFactory.createBgp([
            OperationFactory.createPattern(<RDF.Term> <any> 'x2', <RDF.Term> <any> 'y2', <RDF.Term> <any> 'z2'),
          ]),
          OperationFactory.createBgp([
            OperationFactory.createPattern(<RDF.Term> <any> 'x2O', <RDF.Term> <any> 'y2O', <RDF.Term> <any> 'z2O'),
          ]),
        ),
        OperationFactory.createBgp([
          OperationFactory.createPattern(<RDF.Term> <any> 'a2', <RDF.Term> <any> 'b2', <RDF.Term> <any> 'c2'),
  ...
  describe('#nameToVariable', () => {
    it('should convert a variable with an empty path', async() => {
      ...
      expect(util.nameToVariable('varName', ctx))
  ...
  describe('#valueToNamedNode', () => {
    it('should directly convert a value that is not in the context', async() => {
      expect(util.valueToNamedNode('abc', new JsonLdContextNormalized({})))
        .toEqual(DF.namedNode('abc'));
    });

    it('should expand a value that is in the context', async() => {
      expect(util.valueToNamedNode('abc', new JsonLdContextNormalized({ abc: 'http://example.org/abc' })))
        .toEqual(DF.namedNode('http://example.org/abc'));
    });

    it('should expand a value that is in the context as an object', async() => {
      expect(util.valueToNamedNode('abc', new JsonLdContextNormalized({ abc: { '@id': 'http://example.org/abc' }})))
        .toEqual(DF.namedNode('http://example.org/abc'));
    });
  });
  ...
  describe('#handleNodeValue', () => {
    ...
    it('should convert a variable that is defined', async() => {
      expect(util.handleNodeValue(
        { kind: 'Variable', name: { kind: 'Name', value: 'myVar1' }},
        'va',
        ctx,
      ))
    ...
    it('should convert an int', async() => {
      expect(util.handleNodeValue(
        ...
        .toEqual({ terms: [ DF.literal('123', DF.namedNode('http://www.w3.org/2001/XMLSchema#integer')) ]});
    });
    ...
    it('should convert a float', async() => {
      expect(util.handleNodeValue(
        ...
        .toEqual({ terms: [ DF.literal('123.1', DF.namedNode('http://www.w3.org/2001/XMLSchema#float')) ]});
    });
    ...
    it('should convert a true boolean', async() => {
      expect(util.handleNodeValue(
        ...
        .toEqual({ terms: [ DF.literal('true', DF.namedNode('http://www.w3.org/2001/XMLSchema#boolean')) ]});
    });
    ...
    it('should convert a false boolean', async() => {
      expect(util.handleNodeValue(
        ...
        .toEqual({ terms: [ DF.literal('false', DF.namedNode('http://www.w3.org/2001/XMLSchema#boolean')) ]});
    });
    ...
    it('should convert a list value in RDF-list-mode', async() => {
      ...
      expect(out.terms[0].termType).toBe('BlankNode');
      expect(out.auxiliaryPatterns!).toHaveLength(6);

      expect(out.auxiliaryPatterns![0].subject).toEqual(out.auxiliaryPatterns![1].subject);
      expect(out.auxiliaryPatterns![0].subject.termType).toBe('BlankNode');
      ...
      expect(out.auxiliaryPatterns![1].subject.termType).toBe('BlankNode');
      expect(out.auxiliaryPatterns![1].predicate)
        .toEqual(DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#rest'));
      expect(out.auxiliaryPatterns![1].object.termType).toBe('BlankNode');
      ...
      expect(out.auxiliaryPatterns![2].subject.termType).toBe('BlankNode');
      ...
      expect(out.auxiliaryPatterns![3].subject.termType).toBe('BlankNode');
      expect(out.auxiliaryPatterns![3].predicate)
        .toEqual(DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#rest'));
      expect(out.auxiliaryPatterns![3].object.termType).toBe('BlankNode');
      ...
      expect(out.auxiliaryPatterns![4].subject.termType).toBe('BlankNode');
      expect(out.auxiliaryPatterns![4].predicate)
        .toEqual(DF.namedNode('http://www.w3.org/1999/02/22-rdf-syntax-ns#first'));
      expect(out.auxiliaryPatterns![4].object)
        .toEqual(DF.literal('false', DF.namedNode('http://www.w3.org/2001/XMLSchema#boolean')));
      expect(out.auxiliaryPatterns![5].subject.termType).toBe('BlankNode');
    ...
    it('should convert an object value', async() => {
      ...
      expect(out.terms[0].termType).toBe('BlankNode');
      expect(out.auxiliaryPatterns!).toHaveLength(3);
  ...
  describe('#handleDirectiveNode', () => {
    ...
    it('should return null for an unsupported directive', async() => {
      expect(util.handleDirectiveNode({ directive: unknownDirective, fieldLabel: 'field' }, ctx))
        .toBeNull();
    });

    it('should return true on a true inclusion', async() => {
      expect(util.handleDirectiveNode({ directive: includeTrue, fieldLabel: 'field' }, ctx))
        .toEqual({});
    });

    it('should return false on a false inclusion', async() => {
      expect(util.handleDirectiveNode({ directive: includeFalse, fieldLabel: 'field' }, ctx))
        .toEqual({ ignore: true });
    });

    it('should return false on a true skip', async() => {
      expect(util.handleDirectiveNode({ directive: skipTrue, fieldLabel: 'field' }, ctx))
        .toEqual({ ignore: true });
    });

    it('should return true on a false skip', async() => {
      expect(util.handleDirectiveNode({ directive: skipFalse, fieldLabel: 'field' }, ctx))
        .toEqual({});
    });
    ...
    it('should modify singularize variables and not set the single state on single', async() => {
      ...
      expect(ctx.singularizeState).toBeNull();
    });
  ...
  describe('#createQuadPattern', () => {
    it('should create a triple pattern for a normal context', async() => {
      ...
      expect(util.createQuadPattern(s, p, o, g, new JsonLdContextNormalized({ p: 'ex:myP' })))
    ...
    it('should create a triple pattern for a normal context using @vocab', async() => {
      ...
      expect(util.createQuadPattern(s, p, o, g, context))
        .toEqual(OperationFactory.createPattern(s, DF.namedNode('ex:/myP'), o));
    });
  ...
  describe('#getArgument', () => {
    it('should return null on null arguments', async() => {
      expect(util.getArgument(undefined, 'abc')).toBeUndefined();
    });

    it('should return null on an argument that is not present', async() => {
      expect(util.getArgument([
