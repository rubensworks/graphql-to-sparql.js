...
import type { IVariablesDictionary } from '../lib/IConvertContext';
  ...
  describe('without default data factory', () => {
    ...
    describe('#graphqlToSparqlAlgebra', () => {
      it('it should convert a simple query', async() => {
        ...
        await expect(converter.graphqlToSparqlAlgebra(`
{
  human {
    name
  }
}
`, context)).resolves.toEqual(OperationFactory.createProject(OperationFactory.createBgp([
...
`, context)).resolves.toEqual(OperationFactory.createProject(
...
`, context, { variablesDict })).resolves.toEqual(OperationFactory.createProject(OperationFactory.createBgp([
          OperationFactory.createPattern(
            DF.variable('df_0'),
            DF.namedNode('http://example.org/hero'),
            DF.variable('hero'),
...
`, context, { variablesDict })).resolves.toEqual(OperationFactory.createProject(
          OperationFactory.createJoin([
            OperationFactory.createBgp([
              OperationFactory.createPattern(
                DF.variable('df_0'),
